...
function custom_theme_titles($titleparts) {
    ...
    foreach ($parts as $part) {
        if (isset($titleparts[$part])) {
            $titleparts[$part] = pll__($titleparts[$part]);
        }
    }
    return $titleparts;
}
...
function ajax_get_sizes() {
        ...
        foreach ($data['sizes'] as $keySize => $itemSize):
            ...
            <div class="radio-button-wrap">
                <label class="radio-button radio-button--size r-size-card js-radio-summary">
                    <input type="radio" <?php if ($keySize == $size): ?>checked="checked"<?php endif;
                    ?> name="size" <?php if (!$itemSize['available']): ?>disabled<?php endif; ?> class="picture_input"
    ...
    <div class="delivery-calendar-wrapper">
        <div class="delivery__calendar">
            <div class="datepicker-here-init" data-position="top right" data-language='<?php echo $current_lang; ?>'
                 data-orderDay='10'></div>
            <input type="hidden" name="delivery_date" data-delivery-type-related="" value="<?php echo $deliveryDate; ?>"
                   id="deliveryDate">
        </div>
    </div>
    <?php
...
function ajax_get_edit_order_form_html() {
    ...
    <div class="form-group--select form-group--white form-group--05">
        <label class="select-label select-label-js readonly">
            ...
            <input class="input input-value-js" type="text" name="edit_subject_text"
                   value="<?php echo $dataFields['edit_subject_text']; ?>" readonly
                   placeholder="<?php pll_e('Subject'); ?>" value="<?php ?>" required/>
            <!-- Value of this input will be sent to back -->
            <input class="input input-key-js" name="edit_subject" value="<?php echo $dataFields['edit_subject']; ?>"
    ...
    <div class="form-group--select form-group--white form-group--05">
        <label class="select-label select-label-js">
            ...
            <input class="input input-value-js" type="text" name="edit_choose_tech_text"
                   value="<?php echo $dataFields['edit_choose_tech_text']; ?>" readonly
                   placeholder="<?php pll_e('Style'); ?>" required/>
            <!-- Value of this input will be sent to back -->
            <input class="input input-key-js" name="edit_choose_tech"
                   value="<?php echo $dataFields['edit_choose_tech']; ?>" readonly hidden required>
            <div class="select-label__bottom">
                <?php pll_e('Style'); ?>
            </div>
        </label>
        ...
        <ul class="options options-js" data-list="size">
            <?php foreach ($dataFields['edit_size_options'] as $type => $typeItems): ?>
                <?php foreach ($typeItems as $key => $item): ?>
                    <li class="option option-js"
                        <?php if ($dataFields['edit_choose_tech'] != $type): ?>style="display: none;" <?php endif; ?>
                        data-type="<?php echo $type; ?>" data-key="<?php echo $key; ?>">
                        <div class="option__text"><?php echo $item; ?></div>
                    </li>
                <?php endforeach; ?>
            <?php endforeach; ?>
    ...
    <div class="form-group--select form-group--white form-group--05 background-wrapper">
        <label class="select-label select-label-js">
            ...
            <input class="input input-value-js" type="text" name="edit_background_text"
                   value="<?php echo $dataFields['edit_background_text']; ?>" readonly
                   placeholder="<?php pll_e('Background'); ?>" required/>

            <!-- Value of this input will be sent to back -->
            <input class="input input-key-js" name="edit_background"
                   value="<?php echo $dataFields['edit_background']; ?>" readonly hidden required>
            <div class="select-label__bottom">
        ...
        <ul class="options options-js" data-list="background">
            <?php foreach ($dataFields['edit_background_options'] as $key => $item): ?>
                <li class="option option-js"
                    <?php if ($dataFields['edit_choose_tech'] != 'oil' && $item['type'] == 'color'): ?>style="display: none;" <?php endif; ?>
                    data-type="<?php echo $item['type']; ?>" data-key="<?php echo $key; ?>">
...
// add cart item metadata to order item metadata
...
function change_order_item_meta_title($key, $meta, $item) {
    ...
    if ('_product_gift_amount' == $key) {
        $key = 'Amount';
    }
    if ('_product_gift_currency' == $key) {
        $key = 'Currency';
    }
    if ('_product_gift_sender_name' == $key) {
        $key = 'Sender Name';
    }
    if ('_product_gift_recipient_name' == $key) {
        $key = 'Recipient Name';
    }
    if ('_product_gift_message' == $key) {
        $key = 'Message';
    }
    if ('_product_gift_id' == $key) {
        $key = 'Gift Card ID';
    }
    // picture
    if ('_product_subject' == $key) {
        $key = 'Subject';
    }
    if ('_product_choose_tech' == $key) {
    ...
    if ('_product_photos_count' == $key) {
        $key = 'Photos count';
    }
    if ('_product_frame' == $key) {
        $key = 'Frame';
    }
    if ('_product_frame_selected' == $key) {
        $key = 'Frame selected';
    }
    if ('_product_second_option_to_send_photo' == $key) {
        $key = 'Second option to send us the photo';
    }
    if ('_product_artist_advice' == $key) {
        $key = 'Artist advice';
    }
    if ('_product_upload_comment' == $key) {
        $key = 'Comment';
    }
    if ('_product_duration_type' == $key) {
        $key = 'Duration type';
    }
    if ('_product_delivery_date' == $key) {
        $key = 'Delivery date';
    }
    if ('_product_base_price' == $key) {
        $key = 'Base price';
    }
    if ('_product_frame_price' == $key) {
        $key = 'Frame price';
    }
...
// change order item meta value on admin order page -- not used
...
// woocommerce errors adjustments
add_filter('woocommerce_add_error', function ($message) {
    $message = str_replace('Billing ', '', strip_tags($message));
    $message = str_replace(['Postcode / ZIP', 'Town / City', 'Email address'], ['Postcode', 'City', 'Email'], $message);
    $message = pll__($message);
    return $message;
}, 20, 3);

// display order item metadata -- used in order email
...
// set site currency when create order
...
// when order created check if presents gift cards in order. If present then we creating coupons based on it (createCouponGiftCardID)
...
// create coupon based on gift card
...
function filter_woocommerce_currency($option) {
    if (!is_admin()) {
        $current_lang = pll_current_language();
        $allPricesData = getPrices();
        $site_currency = strtoupper($allPricesData[$current_lang]['currency']);
